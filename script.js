...
const getData = () => {
	...
	fetch("https://api.coingecko.com/api/v3/coins/list")
		...
		.then((json) => {
			cardsData = json.splice(randomNum, 50);
...
const renderModal = () => {
	favorites.forEach((item) => {
		...
		li.innerHTML = `<span class="itemTitle">${item}</span><label class="switch">
...
const uncheckToggles = () => {
	const toggles = cardsContainer.querySelectorAll(
...
const createGraph = () => {
	...
	const getPoints = async (array) => {
		if (array.length > 0) {
			...
			Object.values(object)[1]
				...
				: "";
			Object.values(object)[2]
				? coin3.push({
						x: new Date(),
						y: Object.values(object)[2].USD,
				  })
				: "";
			Object.values(object)[3]
				? coin4.push({
						x: new Date(),
						y: Object.values(object)[3].USD,
				  })
				: "";
			Object.values(object)[4]
				? coin5.push({
						x: new Date(),
	...
	let chart = new CanvasJS.Chart("chartContainer", {
		title: {
			text: `${
				favorites.length === 0
					? "Please choose any currency to heed its cost in "
					: favorites + " to"
			} USD`,
		},
		axisX: {
		...
		data: [
			{
				...
				showInLegend: favorites[0] ? true : false,
				dataPoints: coin1,
			},
			{
				type: "line",
				legendText: coin2 ? favorites[1] : "",
				showInLegend: favorites[1] ? true : false,
				dataPoints: coin2,
			},
			{
				type: "line",
				legendText: coin3 ? favorites[2] : "",
				showInLegend: favorites[2] ? true : false,
				dataPoints: coin3,
			},
			{
				type: "line",
				legendText: coin4 ? favorites[3] : "",
				showInLegend: favorites[3] ? true : false,
				dataPoints: coin4,
			},
			{
				type: "line",
				legendText: coin5 ? favorites[4] : "",
				showInLegend: favorites[4] ? true : false,
				dataPoints: coin5,
			},
		],
	});
